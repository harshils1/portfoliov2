...
export default function Projects() {
  ...
  const renderCard = (card) => {
    return (
      <Card style={{ width: "20rem" , background: "black"}} className='card' >
        <Card.Img variant="top" src={card.image} />
        <Card.Body>
          <Card.Title style={{color: "white"}}>{card.title}</Card.Title>
          <Card.Text style={{color: "white"}}>{card.text}</Card.Text>
          <Button variant="primary" className="card-button"><a className="btn-ins" href={card.link} style={{color: "white", decoration: "none"}}>View Here</a></Button>
        </Card.Body>
      </Card>
    );
  };
